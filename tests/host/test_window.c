...
int main(void) {
    ...
    lg_window_init(&w, text, sizeof text, widths, 0, 0, 0, 0, capture, &seen);
    ...
    lg_window_init(&w, plain, sizeof plain, widths, 0, 0, 0, 0, capture, &seen);
    ...
    lg_window_init(&w, truncated, sizeof truncated, widths, 0, 0, 0, 0, capture, &seen);
    ...
    lg_window_init(&w, abc, sizeof abc, widths, 0, 0, 12, 0, capture, &seen);
    assert(lg_window_step(&w, false) == LG_WINDOW_DONE);
    assert(seen.count == 3);
    ...
    lg_window_init(&w, twolines, sizeof twolines, widths, 0, 0, 0, 1, capture, &seen);
    ...
    lg_window_init(&w, 0, 0, widths, 0, 0, 0, 0, capture, &seen);
