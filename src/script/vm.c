...
enum { OP_END=0x00, OP_SET=0x01, OP_ADD=0x02, OP_WAIT=0x03, OP_TEXT=0x04,
       ...
       OP_ITEM=0x09, OP_ITEM_TAKE=0x0a };
...
void lg_script_set_item_take_fn(LGScriptVM *v,LGScriptItemFn fn,void *context){if(!v)return;v->item_take_fn=fn;v->item_take_context=context;}
LGScriptStatus lg_script_step(LGScriptVM *v){
 ...
 case OP_ITEM_TAKE:{if(v->size-v->pc<3){v->status=LG_SCRIPT_ERROR;break;}uint8_t id=v->code[v->pc++];uint16_t qty=v->code[v->pc];qty|=(uint16_t)((uint16_t)v->code[v->pc+1]<<8);v->pc+=2;if(!v->item_take_fn||!v->item_take_fn(v->item_take_context,id,qty)){v->status=LG_SCRIPT_ERROR;break;}v->status=LG_SCRIPT_BLOCKED;break;}
